    ...
    <transition>
      <div class="chatbox" v-show="show">
        <div class="chatbox__header">
          OGANI
          <span class="chatbox__toggle" @click="show = !show"
            ><i class="fa fa-close" />
          </span>
        </div>
        <div class="chatbox__body">
          ...
          <div class="chat__logs">
            <div
              :id="`cm-msg-${i}`"
              v-for="(message, i) in messageArr"
              :key="i"
              :class="`chatbox__msg ${message.type}`"
            >
              <div
                class="cm-msg-text"
                :class="{ 'cm-msg-item': typeof message.content === 'object' }"
              >
                <div
                  v-if="typeof message.content === 'object'"
                  class="chatbox__item-content cursor-pointer"
                  @click="openNewTab(message.content)"
                >
                  <img
                    :alt="message.content?.name"
                    :src="message.content?.list_img?.[0]?.url"
                  />
...
export default defineComponent({
  ...
  mounted() {
    document.body.addEventListener(
      "show-chat-item",
      (e: any) => this.showItemInChat(e.detail.chatData),
      true
    );
  },
  updated() {
